...
This build adds:
- Dark Mode + user-theming via bug-notes-config.json (hex colors + transparency)
- View menu: toggle Dark Mode and Reload Theme from JSON
- Theming applied after wizard/main screen swaps; classic widgets (Text/Listbox) colored too
- All prior features from updated3 build retained

If bug-notes-config.json isn't present, sensible defaults are used.
"""
...
# ---- Config / Theme ----
CONFIG_PATH = Path("bug-notes-config.json")
...
class ConfigManager:
    ...
    def load(self):
        try:
            ...
            # Ignore bad/missing config; stick to defaults
            pass
    ...
    def is_dark(self) -> bool:
        return (self.data.get("theme") or "dark").lower() == "dark"

    def palette(self) -> dict:
        colors = self.data.get("colors", {})
        key = "dark" if self.is_dark() else "light"
        pal = dict(colors.get(key, {}))
        defaults = {
            "bg":        "#111418" if key=="dark" else "#F7F7F9",
            "fg":        "#E6E6E6" if key=="dark" else "#1F2328",
            "panel":     "#171B20" if key=="dark" else "#FFFFFF",
            "accent":    "#6EA8FE" if key=="dark" else "#0D6EFD",
            "muted":     "#9AA4AF" if key=="dark" else "#6C757D",
            "entry_bg":  "#0E1116" if key=="dark" else "#FFFFFF",
            "entry_fg":  "#E6E6E6" if key=="dark" else "#1F2328",
            "list_bg":   "#0E1116" if key=="dark" else "#FFFFFF",
            "list_fg":   "#E6E6E6" if key=="dark" else "#1F2328",
            "button_bg": "#20252B" if key=="dark" else "#E9ECEF",
            "button_fg": "#E6E6E6" if key=="dark" else "#1F2328",
            "separator": "#2A3139" if key=="dark" else "#DEE2E6",
            "select_bg": "#22324A" if key=="dark" else "#CCE5FF",
            "select_fg": "#FFFFFF" if key=="dark" else "#0B3D91",
            "link":      "#86B7FE" if key=="dark" else "#0D6EFD",
        }
        for k, v in defaults.items():
            pal.setdefault(k, v)
        return pal

def apply_theme(root: "tk.Tk", style: "ttk.Style", cfg: ConfigManager):
    """Apply a cross-platform theme using 'clam' so custom colors take effect."""
    pal = cfg.palette()
    try:
    ...
    # Window transparency
    try:
        alpha = float(cfg.data.get("transparency", 1.0))
    ...
    bg = pal["bg"]; fg = pal["fg"]
    panel = pal["panel"]; accent = pal["accent"]
    entry_bg = pal["entry_bg"]; entry_fg = pal["entry_fg"]
    list_bg = pal["list_bg"]; list_fg = pal["list_fg"]
    btn_bg = pal["button_bg"]; btn_fg = pal["button_fg"]
    sep = pal["separator"]; selbg = pal["select_bg"]; selfg = pal["select_fg"]

    # Classic tk defaults (affect Text, Listbox, etc.)
    root.option_clear()
    root.option_add("*Background", panel)
    ...
    style.configure("TNotebook", background=panel, foreground=fg, bordercolor=panel)
    ...
    style.configure("TButton", background=btn_bg, foreground=btn_fg,
                    focuscolor=panel, bordercolor=btn_bg, padding=(10,4))
    style.map("TButton",
    ...
    # Apply to existing children (classic widgets)
        ...
        for w in widget.winfo_children():
            try:
                if isinstance(w, tk.Text):
                    ...
                    w.configure(bg=list_bg, fg=list_fg, selectbackground=selbg, selectforeground=selfg)
                elif isinstance(w, tk.Canvas):
                    w.configure(bg=panel)
            except Exception:
                pass
            _apply_to_children(w)
    _apply_to_children(root)

def toggle_theme_and_save(root, style, cfg: ConfigManager, to_dark: bool):
    cfg.data["theme"] = "dark" if to_dark else "light"
    cfg.save()
    apply_theme(root, style, cfg)
...
def write_bug_content(text: str, bug_id: str, new_summary: str,
    ...
    for i, m in enumerate(metas):
        ...
        platform = _grab_line(block, "Platform")
...
if TK_AVAILABLE:
    ...
    class NewFileDialog(tk.Toplevel):
        def __init__(self, master, on_done):
            ...
            for p in ALL_PLATFORMS:
                ...
                ttk.Entry(frm, textvariable=v, width=14).grid(row=row, column=1, sticky="w", padx=6, pady=1)
    ...
    class MainApp(ttk.Frame):
        ...
        def change_file(self):
            ...
            # Reapply theme to new/updated widgets
            apply_theme(self.winfo_toplevel(), self.winfo_toplevel().style, self.winfo_toplevel().cfg)
    ...
    class HeaderTab(ttk.Frame):
        def __init__(self, master, app: MainApp):
            ...
            for p in ALL_PLATFORMS:
                ...
                v=tk.StringVar(value="")
                ttk.Entry(grid,textvariable=v,width=14).grid(row=r,column=1,sticky="w",padx=6,pady=2)
    ...
    class App(tk.Tk):
        def __init__(self):
            ...
            apply_theme(self, self.style, self.cfg)
            ...
            self.darkmode_var = tk.BooleanVar(value=self.cfg.is_dark())
            viewm.add_checkbutton(label="Dark Mode", onvalue=True, offvalue=False,
                                  variable=self.darkmode_var,
                                  command=lambda: toggle_theme_and_save(self, self.style, self.cfg, self.darkmode_var.get()))
            viewm.add_command(label="Reload Theme from JSON", command=self.reload_theme_from_disk)
        ...
        def reload_theme_from_disk(self):
            self.cfg.load()
            self.darkmode_var.set(self.cfg.is_dark())
            apply_theme(self, self.style, self.cfg)
        ...
        def show_wizard(self):
            ...
            # Reapply theme to new wizard widgets
            apply_theme(self, self.style, self.cfg)
