...
This build:
- Single-click selection (no Ctrl) for Issues and Bugs selection lists via WrappedListboxSingle (Listbox, selectmode='browse').
- Wrapped list items that reflow on resize.
- Bugs tab auto-fills default steps based on gen4/gen5 when an issue is clicked.
- Bugs written tab: Username field + Copy, resizable list/detail via PanedWindow, left-aligned buttons, "Report" label,
  and a new "Copy [bug#] + Summary" button that copies like: [1245] this is a summary of the issue.
- Full header editor, Issues add/move/remove, Found/Invalid tagging & copy, bug generation & backups.
- Cross-platform clipboard (macOS pbcopy, Windows clip, fallback Tk).
- CLI mode still available via --cli.
"""
import sys
import re
import hashlib
...
def write_bug_meta_bugnum(text: str, bug_id: str, new_bugnum: str | None) -> str:
    ...
    return re.sub(BUG_META_RE, lambda m: repl(m) if m.group(1) == bug_id else m.group(0), text)
...
def append_bug_and_move_issue(text: str, ev: dict, summary_prefix: str,
    ...
    steps_block = build_steps_block_numbered(template_key, steps_mode, plain_lines)

    bug_block = make_bug_block(summary, ev["platform"], username, steps_block, observed or "", expected or "")
...
if TK_AVAILABLE:
    class WrappedListboxSingle(ttk.Frame):
        """
        A single-select Listbox that visually wraps long items to multiple lines
        and rewraps on resize. Click anywhere on a wrapped item to select it.
        """
        def __init__(self, master, wrap_margin_chars=80, **kwargs):
            ...
            self.items = []          # original item strings
            self.lines_map = []      # visual line index -> item index
            self.blank_between = True
        ...
        def _calc_wrap_chars(self):
            # approximate wrap width by widget width / average char width
            try:
        ...
        def _render(self):
            ...
            for idx, text in enumerate(self.items):
                ...
                if self.blank_between and idx != len(self.items) - 1:
                    lb.insert("end", "")          # visual separator
                    self.lines_map.append(None)    # None = separator
            # clear selection
            if lb.size() > 0:
        ...
        def _on_select(self, _evt):
            # normalize selection to the first line of the clicked item
            ...
            if item_idx is None:
                if self.on_change: self.on_change(None)
                return
            # move highlight to the representative (first) visual line for that item
            first_vis = self._first_visual_for_item(item_idx)
            self.listbox.selection_clear(0, "end")
            self.listbox.selection_set(first_vis)
            self.listbox.activate(first_vis)
            if self.on_change: self.on_change(item_idx)

        def _find_item_from_visual(self, vis_idx):
            if 0 <= vis_idx < len(self.lines_map):
                return self.lines_map[vis_idx]
            return None
    ...
    class NewFileDialog(tk.Toplevel):
        def __init__(self, master, on_done):
            ...
            for p in ALL_PLATFORMS:
                ...
                v=tk.StringVar(value=""); ttk.Entry(frm, textvariable=v, width=20).grid(row=row, column=1, sticky="we", padx=6, pady=1)
    ...
    class HeaderTab(ttk.Frame):
        def __init__(self, master, app: MainApp):
            ...
            for p in ALL_PLATFORMS:
                ...
                v=tk.StringVar(value=""); ttk.Entry(grid,textvariable=v,width=20).grid(row=r,column=1,sticky="we",padx=6,pady=2)
                grid.columnconfigure(1, weight=1); self.user_vars[p]=v; r+=1
    ...
    class IssuesTab(ttk.Frame):
        def __init__(self, master, app: MainApp):
            ...
            ttk.Label(left, text="Issues (timestamped notes inside 'issues found 🕵️‍♂️')").grid(row=0,column=0,sticky="w")
            ...
            btns=ttk.Frame(left); btns.grid(row=2,column=0,sticky="w", pady=(0,6))
    ...
    class FoundTab(ttk.Frame):
        def __init__(self, master, app: MainApp):
            ...
            pan=ttk.PanedWindow(self, orient="horizontal"); grid_fill(pan,0,0, padx=8, pady=8)
            left=ttk.Frame(pan); right=ttk.LabelFrame(pan, text="Tag editor / Copy")
            pan.add(left, weight=3); pan.add(right, weight=2)

            left.grid_columnconfigure(0, weight=1); left.grid_rowconfigure(1, weight=1)
            ttk.Label(left, text="Found / Invalid issues:").grid(row=0,column=0,sticky="w")
            self.listbox=tk.Listbox(left, selectmode="browse", exportselection=False, activestyle="none")
            self.listbox.grid(row=1,column=0,sticky="nsew",pady=(4,8))
            ...
            ttk.Button(rowbtns,text="Set/Update Tag",command=self.set_tag).pack(side="left",padx=4)
            ttk.Button(rowbtns,text="Remove Tag",command=self.clear_tag).pack(side="left",padx=4); r+=1

            ttk.Button(right, text="Copy", command=self.copy_markdown_strike).grid(row=r,column=1,sticky="w",padx=8,pady=(0,10))

            self.list=[]; self.refresh()

        def refresh(self):
            self.listbox.delete(0,"end"); self.list=[]
            if not self.app.notes_path.exists():
                self.sel_var.set(""); self.bugnum_var.set(""); return
            text=self.app.notes_path.read_text(encoding="utf-8"); text=find_or_create_sections(text)
            self.list=found_list(text)
            for it in self.list:
                label=(f"[{it['bugnum']}] " if it["bugnum"] else "") + f"[{it['time']}][{it['platform']}] {it['desc']}"
                self.listbox.insert("end", label)
            self.sel_var.set(""); self.bugnum_var.set("")

        def on_select(self,_=None):
            sels=self.listbox.curselection()
            if not sels: self.sel_var.set(""); self.bugnum_var.set(""); return
            it=self.list[sels[0]]
        ...
        def clear_tag(self):
            ...
            try:
                text=self.app.notes_path.read_text(encoding="utf-8")
                new_text=self._rewrite_found_line(text, it, None)
                self.app.notes_path.write_text(new_text, encoding="utf-8")
                self.refresh()
            except Exception as e:
                messagebox.showerror("Error", str(e))
    ...
    class BugsTab(ttk.Frame):
        def __init__(self, master, app: MainApp):
            ...
            self.wlist=WrappedListboxSingle(left)
            self.wlist.grid(row=1,column=0,sticky="nsew",pady=(4,4))
            ...
            ttk.Label(right, text="Steps (plain lines; editing switches to Custom):").grid(row=r,column=0,sticky="nw",padx=8,pady=(8,2))
    ...
    class BugsWrittenTab(ttk.Frame):
        def __init__(self, master, app: MainApp):
            ...
            # Left: wrapped single-select list of written bugs
            left = ttk.Frame(pan)
            left.grid_columnconfigure(0, weight=1); left.grid_rowconfigure(1, weight=1)
            ...
            r = 0
            ttk.Label(right, text="Selected summary:").grid(row=r, column=0, sticky="w", padx=6, pady=(6, 2))
            ...
            # Existing "Copy" (kept): copies "[bug#] summary"
            ttk.Button(sum_btns, text="Copy", command=self.copy_summary_with_bug).pack(side="top", anchor="w", pady=(0,4))
            # New button: explicit label
            ttk.Button(sum_btns, text="Copy [bug#] + Summary", command=self.copy_bugnum_and_summary).pack(side="top", anchor="w")
            r += 1
            ...
            ttk.Label(right, text="Bug number:").grid(row=r, column=0, sticky="w", padx=6, pady=(6, 2))
            self.bugnum_var = tk.StringVar(value="")
            self.bugnum_entry = ttk.Entry(right, textvariable=self.bugnum_var, width=20)
            self.bugnum_entry.grid(row=r, column=1, sticky="w", padx=6, pady=(6, 2))
            rowbtns = ttk.Frame(right); rowbtns.grid(row=r, column=2, sticky="w", padx=6)
            ttk.Button(rowbtns, text="Set/Update", command=self.set_bugnum).pack(side="left", padx=(0, 6))
            ttk.Button(rowbtns, text="Clear", command=self.clear_bugnum).pack(side="left")
            r += 1
            ...
            ttk.Button(rep_btns, text="Copy", command=lambda: copy_to_system(self.report_txt.get("1.0", "end").strip())).pack(side="left")
            r += 1
        ...
        def refresh(self):
            ...
            labels=[]
            for b in self.bugs:
                left_tag=f"[{b['bugnum']}]" if b["bugnum"] else "[]"
                labels.append(f"{left_tag}[{b['platform']}] {b['summary'] or '(no summary)'}")
            self.wlist.set_items(labels)
            self.selected_idx=None; self._clear_fields()

        def _clear_fields(self):
            self.summary_var.set(""); self._set_report(""); self.bugnum_var.set(""); self.user_val.set("")
        ...
        def _update_selection(self, idx):
            ...
            self.user_val.set(bug["username"])
        ...
        def clear_bugnum(self):
            if self.selected_idx is None: return
            bug=self.bugs[self.selected_idx]
            try:
                txt=self.app.notes_path.read_text(encoding="utf-8")
                new_txt=write_bug_meta_bugnum(txt, bug["id"], None)
                self.app.notes_path.write_text(new_txt, encoding="utf-8")
                self.refresh()
            except Exception as e:
                messagebox.showerror("Error", str(e))

        def copy_summary_with_bug(self):
            if self.selected_idx is None: return
            bug=self.bugs[self.selected_idx]
            tag=f"[{bug['bugnum']}]" if bug["bugnum"] else "[]"
            copy_to_system(f"{tag} {bug['summary']}")

        # NEW: Copies exactly "[bug#] summary"
